...
// Fullscreen quad from vertex_id (two triangles). drawableSize in pixels.
...
fragment float4 orbFragment(OrbVSOut in [[stage_in]],
    ...
    // Gaussian: bright at center, ~0 at the edge radius.
    float a = exp(-(d * d) / (u.radius * u.radius)) * u.opacity * u.globalOpacity;
    // Premultiplied color; additive pipeline blend stacks mint+violet glow.
