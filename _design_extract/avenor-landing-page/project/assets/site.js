...
(function () {
  ...
  function typewriteInto(typedEl, text, perChar = 80) {
  ...
  function bindDemo(host) {
    ...
    async function run(text, freeform = false) {
      ...
      await typewriteInto(typedEl, text, 80);
      if (myToken !== token) { busy = false; return; }
      await new Promise(r => setTimeout(r, 600));
      if (myToken !== token) { busy = false; return; }
      const parsed = freeform
        ? parseFreeForm(text)
        : PARSE_PRESETS[text] || parseFreeForm(text);
      renderParse(host, parsed);
      busy = false;
    }

    buttons.forEach(btn => {
      btn.addEventListener('click', () => run(btn.getAttribute('data-text'), false));
    });
    ...
    if (inputEl) {
      ...
      inputEl.addEventListener('input', () => {
        ...
        t = setTimeout(() => {
          ...
          if (parsed) renderParse(host, parsed);
        }, 400);
      });
    }
  }
